...
<section class="content-header">
  <h1>
    {{ Request::get('label') == 'stock_in' ? __('title.stock_in') : __('title.stock_out') }} <small>#{{ $stock_movement->id }}</small>
  </h1>
  <ol class="breadcrumb">
    ...
    <div class="col-md-12">
      <div style="padding-bottom: 10px">
        @if(Auth::user()->allowEdit(config('global.modules.stock')))
          @if($is_done == 0)
            <button id="submit-button" type="button" class="btn btn-primary"><i class="fa fa-save"></i>&nbsp; {{ __('title.save_changes') }}</button>
          @else
