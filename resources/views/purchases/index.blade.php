...
<section class="content-header">
  <h1>
    {{ __('title.dashboard') }}
    <small>{{ __('title.list_purchase_orders') }}</small>
  </h1>
  <ol class="breadcrumb">
    <li><a href="{{ route('dashboard') }}"><i class="fa fa-dashboard"></i>{{ __('title.dashboard') }}</a></li>
    <li class="active">{{ __('title.purchase_orders') }}</li>
  </ol>
</section>
    ...
    <div class="col-lg-12 col-md-12">
          <div class="box">
            <div class="box-header with-border">
              <h3 class="box-title">{{ __('title.list_purchase_orders') }}</h3>
              <!-- tools box - add new record-->
              @if(Auth::user()->allowCreate(config('global.modules.purchase')))
              <div class="pull-right box-tools">
                <a href="{{ route('purchase.create') }}" class="btn btn-primary btn-sm" data-widget="add-new" data-toggle="tooltip"
              ...
              <table class="table table-striped table-hover">
               ...
               {{--    @foreach($roles as $key => $item)
                  <tr>
                    ...
                    <td>
                      @if(Auth::user()->allowEdit(config('global.modules.purchase')))
                      <a href="{{ route('purchase.edit', $item->id) }}" title="Edit"><i class="fa fa-pencil"></i></a>
                      @endif                      
                      @if(Auth::user()->allowDelete(config('global.modules.purchase')))                      
                        ...
                        <div class="modal-dialog">
                          <div class="modal-content">
                            ...
                            <div class="modal-footer">
                              <form action="{{ route('purchase.delete', $item->id) }}" method="POST">
                                @csrf
