...
<section class="content-header">
  <h1>
    {{ __('title.dashboard') }}
    <small>{{ __('title.list_dimension_groups') }}</small>
  </h1>
  <ol class="breadcrumb">
    <li><a href="{{ route('dashboard') }}"><i class="fa fa-dashboard"></i>{{ __('title.dashboard') }}</a></li>
    <li class="active">{{ __('title.dimension_groups') }}</li>
  </ol>
</section>
    ...
    <div class="col-lg-12 col-md-12">
          <div class="box">
            <div class="box-header with-border">
              <h3 class="box-title">{{ __('title.list_dimension_groups') }}</h3>
              <!-- tools box - add new record-->
              @if(Auth::user()->allowCreate(config('global.modules.dimension')))
              <div class="pull-right box-tools">
                <a href="{{ route('dimension-group.create') }}" class="btn btn-primary btn-sm" data-widget="add-new" data-toggle="tooltip"
              ...
              <table class="table table-striped table-hover">
                <thead>
                  <tr>
                    ...
                    <th>{{ __('app.dimension_group') }}</th>
                    <th>{{ __('app.dimension') }}</th>
                ...
                <tbody>
                  ...
                  <tr>
                    <td>{{ $key+1 }} </td>
                    <td>{{ $item->dimension_group }}</td>
                    <td>
                      @foreach($item->details as $d)                        
                        <span class="label label-success">{{ $d->dimension->dimension_name }}</span>
                      @endforeach
                    ...
                    <td>
                      ...
                      <a href="{{ route('dimension-group.edit', $item->id) }}" title="Edit"><i class="fa fa-pencil"></i></a>
                      @endif                      
                      @if(Auth::user()->allowDelete(config('global.modules.dimension')))                      
                        ...
                        <div class="modal-dialog">
                          <div class="modal-content">
                            <div class="modal-header">
                              ...
                              <h4 class="modal-title">{{ __('title.delete') }} - {{$item->dimension_name}}</h4>
                            </div>
                            ...
                            <div class="modal-footer">
                              <form action="{{ route('dimension-group.delete', $item->id) }}" method="POST">
                                @csrf
            ...
            <div class="box-footer clearfix">
              @include('layouts.pagination', ['data'=>$groups])
            </div>             
          </div>      
    </div>
  </div>
