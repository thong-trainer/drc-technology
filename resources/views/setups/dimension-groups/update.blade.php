...
<section class="content-header">
  <h1>
    {{ __('title.edit_dimension_group') }} <small>#{{ $group->id }}</small>
  </h1>
  <ol class="breadcrumb">
  ...
  <div class="row">
    <div class="col-md-12">
      <div class="box">
        ...
        <!-- /.box-header -->
        <div class="box-body">            
          <form role="form" action="{{ route('dimension-group.update', $group->id) }}" method="POST">
            @csrf @method('PUT')
            <div class="box-body">    
             <div class="row">
              <div class="col-lg-6 col-md-8 col-sm-12">
                ...
                <div class="form-group @error('dimension_group') has-error @enderror">
                  ...
                  <input type="text" class="form-control" id="dimension" name="dimension_group" value="{{ old('dimension_group')?:$group->dimension_group }}" required>
                  ...
                  <select class="form-control select2" multiple="multiple" id="dimensions" data-placeholder="{{ __('app.select_dimensions') }}"
                    ...
                    @if(old('dimensions'))
                      @foreach($dimensions as $item)                   
                          @if( in_array($item->id, old('dimensions')) )
                    ...
                    @else
                      ...
                      @foreach($dimensions as $item)
                          @if( $group->details->where('dimension_id', $item->id)->first() )
                            <option value="{{ $item->id }}" selected >{{ $item->dimension_name }}</option>                    
                          @else
                            <option value="{{ $item->id }}" >{{ $item->dimension_name }}</option>
                          @endif                
                ...
                <div class="form-group">
                  ...
                  <textarea type="text" rows="5" class="form-control" id="description" name="description">{{ old('description')?:$group->description }}</textarea>                 
                </div>                                  
              </div>                
