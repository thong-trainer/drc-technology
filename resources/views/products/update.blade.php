...
<section class="content-header">
  <h1>
    {{ __('title.edit_product') }} <small>#{{ $product->id }}</small>
  </h1>
  <ol class="breadcrumb">
      ...
      <div class="box">
        ...
        <div class="box-body">            
          <form role="form" action="{{ route('product.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            @csrf @method('PUT')
            <input type="hidden" name="sale_price" value="{{ $product->salePrice() }}">
            <div class="box-body">    
             <div class="row">
              <div class="col-lg-6 col-md-12 col-sm-12">
                ...
                <div class="row">
                  ...
                  <div class="col-lg-8 col-md-8 col-sm-12">
                    <div class="form-group @error('product_type') has-error @enderror">
                      <label>
                        <input type="radio" value="storable" name="product_type" class="flat-red" @if($product->product_type == 'storable') checked @endif>
                        {{ __('app.storable') }}
                      </label>&nbsp;&nbsp;&nbsp;                                      
                      <label>
                        <input type="radio" value="service" name="product_type" class="flat-red" @if($product->product_type == 'service') checked @endif>
                        {{ __('app.service') }}
                      </label>
                    </div>          
                    <div class="form-group @error('product_name') has-error @enderror">
                      <input type="text" class="form-control big-input" id="name" name="product_name" value="{{ old('product_name') ?: $product->product_name }}" placeholder="{{ __('app.name') }}" required>
                      @error('product_name')
                      ...
                      <div class="col-lg-6 col-md-6 col-sm-12">
                        <div class="form-group @error('category_id') has-error @enderror">
                          <select class="form-control select2" name="category_id">
                            ...
                            <option value="{{ $item->id }}" @if(old('category_id', $product->category_id) == $item->id) selected @endif>{{ $item->category_name }}</option>
                            @endforeach
                          </select>   
                          @error('category_id')
                          <span class="help-block">{{ $message }}</span>
                      ...
                      <div class="col-lg-6 col-md-6 col-sm-12">
                        <div class="form-group @error('dimension_group_id') has-error @enderror">
                          <select class="form-control select2" name="dimension_group_id">
                            ...
                            <option value="{{ $item->id }}" @if(old('dimension_group_id', $product->dimension_group_id) == $item->id) selected @endif>{{ $item->dimension_group }}</option>
                            @endforeach
                          </select>   
                      ...
                      <div class="col-lg-6 col-md-6 col-sm-12">
                        <div class="form-group @error('cost') has-error @enderror">
                          ...
                          <div class="input-group">
                            ...
                            <input type="number" step="any" id="cost" name="cost" class="form-control" value="{{ old('cost') ?: $product->cost }}" min="0" required>
                          </div>                         
                          @error('cost')
                          <span class="help-block">{{ $message }}</span>
                          @enderror                          
                ...
                <div class="form-group">
                  ...
                  <textarea type="text" rows="6" class="form-control" id="notes" name="note">{{ old('notes') ?: $product->notes }}</textarea>
                </div>     
              </div>
                  ...
                  <div class="col-lg-6 col-md-6 col-sm-12">
                    ...
                    <div class="form-group">
                      ...
                      <div class="input-group">
                        ...
                        <input type="text" id="ref_number" name="ref_number" class="form-control" value="{{ old('ref_number') ?: $product->ref_number }}">
                      </div>                            
                    </div>   
                    <div class="form-group @error('customer_tax') has-error @enderror">
                      ...
                      <div class="input-group">
                        ...
                        <input type="number" id="customer_tax" name="customer_tax" class="form-control" value="{{ old('customer_tax') ?: $product->customer_tax }}" min="0" max="100" >
                      </div>                                                 
                    </div>                                                           
                    <div class="form-group @error('sale_unit_id') has-error @enderror">
                      <label for="sale_unit_id">{{ __('app.sale_unit') }}</label>
                      <select class="form-control select2" id="sale_unit_id" name="sale_unit_id">
                        @foreach($units as $item)
                        <option value="{{ $item->id }}" @if(old('sale_unit_id', $product->sale_unit_id) == $item->id) selected @endif>{{ $item->unit_name }}</option>
                        @endforeach
                      </select>                                           
                    </div> 
                    <div class="form-group">
                      <label>
                        <input type="checkbox" class="flat-red" value="1" name="is_pos" @if($product->is_pos == 1) checked @endif>
